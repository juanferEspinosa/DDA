...
def average_vectors(data):
    initial_time = MPI.Wtime()
    a, b = data
    z = []
    suma = [(a[i]+b[i])/2 for i in range(len(a))] 
    z.append(suma)
...
if worker == 0:
    ...
    final_output = []
    adding, time = average_vectors(data1)
    final_output.append(adding)
    for i in range(1, num_workers):
        output, time_worker = comm.recv()
        final_output.append(output)
        time += time_worker
    #print('final array', final_output) # Print of final array
    print('Final time', time)

else:
    data = comm.recv(source=0)
    adding, time = average_vectors(data)
    info = (adding, time)
